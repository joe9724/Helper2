...
public class MyHelpPeopleActvitity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.myhelp_people_activity);
		ImageView iv_back;
...
//设置 Header 为 Material风格
	...
	private void initData(int pageIndex) {
		...
		client.get(Urls.mypeople+MyApplication.workerId+"&pageIndex="+pageIndex, new AsyncHttpResponseHandler() {
				...
				if(Utils.byteArrayToStr(responseBody)!=null)
				{
					try {
						...
						try {
							...
							Toast.makeText(MyHelpPeopleActvitity.this,e.getMessage(),Toast.LENGTH_SHORT).show();
						}
					}catch (Exception e)
					{
                        lists = new ArrayList<MyServerItem>();
			...
			public void onFailure(int statusCode, cz.msebera.android.httpclient.Header[] headers, byte[] responseBody, Throwable error) {
                Toasty.success(MyHelpPeopleActvitity.this, "请求失败!", Toast.LENGTH_SHORT, true).show();
			}
		});
	...
	class LayoutAdapter extends RecyclerView.Adapter<LayoutAdapter.myViewholder> {
		...
		public LayoutAdapter.myViewholder onCreateViewHolder(ViewGroup parent, int viewType) {
			myViewholder = new myViewholder(LayoutInflater.from(MyHelpPeopleActvitity.this).inflate(R.layout.item_layout_myhelper, parent, false));
			return myViewholder;
		}
		...
		public void onBindViewHolder(LayoutAdapter.myViewholder holder, final int position) {
			...
			Glide.with(MyHelpPeopleActvitity.this).load(Urls.prefix+"img/"+lists.get(position).getAvatar()).placeholder(R.drawable.defaultavatar).error(R.drawable.defaultavatar).centerCrop().crossFade().into(holder.iv_avatar);

			holder.item.setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View v) {
					Intent intent = new Intent(MyHelpPeopleActvitity.this,HealthPickerEdit.class);
					intent.putExtra("item", (Serializable)lists.get(position));
					MyHelpPeopleActvitity.this.startActivity(intent);
				}
			});
		}
